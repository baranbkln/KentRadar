alter table public.issue_user_reports
  add column if not exists withdrawn_at timestamptz;

create index if not exists issue_user_reports_active_issue_idx
  on public.issue_user_reports (issue_id, last_reported_at desc)
...
begin
  ...
  where ri.category = p_category
    and ri.status in ('new', 'verified', 'active', 'stale', 'disputed')
...
revoke all on function public.withdraw_issue_report(uuid) from public, anon, authenticated;

grant execute on function public.get_issue_user_state(uuid) to authenticated;
grant execute on function public.withdraw_issue_report(uuid) to authenticated;
