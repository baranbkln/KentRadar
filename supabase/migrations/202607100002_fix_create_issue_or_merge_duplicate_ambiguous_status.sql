...
begin
  ...
  if v_existing_issue_id is not null then
    ...
    v_issue_status := public.apply_road_issue_status(v_existing_issue_id);
    ...
    select
      ...
      'Benzer bir yol sorunu burada zaten var. Bildirimin mevcut kayda eklendi.'::text as message;
  ...
  returning ri.id, ri.status into v_new_issue_id, v_issue_status;
  ...
  select
    ...
    false as merged,
    v_issue_status as status,
    0::numeric as distance_to_issue_meters,
    'Yol sorunu kaydedildi.'::text as message;
end;
$$;
...
begin
  ...
  where ri.id = p_issue_id
  returning ri.status into v_issue_status;
  ...
  select
    ...
    v_issue_status as status,
    v_distance_meters as distance_to_issue_meters,
    'Araç hasarı bildirimi kaydedildi.'::text as message;
end;
$$;
...
begin
  ...
  select
    ...
    v_issue_status as status,
    v_distance_meters as distance_to_issue_meters,
    'Çözülmüş olarak bildirildi.'::text as message;
end;
$$;
...
begin
  ...
  select
    ...
    v_issue_status as status,
    v_distance_meters as distance_to_issue_meters,
    'Yanlış konum veya kayıt bildirimi kaydedildi.'::text as message;
end;
$$;
